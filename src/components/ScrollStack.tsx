...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    window.addEventListener('resize', recalculateTops, { passive: true });
    ...
    return () => {
      ...
      window.removeEventListener('resize', recalculateTops);
      stackCompletedRef.current = false;
      cardsRef.current = [];
  ...
  useEffect(() => {
    const t1 = setTimeout(recalculateTops, 300);
    const t2 = setTimeout(recalculateTops, 1000);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
    };
